...
class Post {
    ...
    public static function findNumberOfPosts(userId : DataIdentifier, identifier : String, fn : DataError -> Int -> Void) : Void {
		...
		Data.query('SELECT COUNT(DISTINCT posts.id) AS count FROM posts ' + ftables + ' WHERE status ' + fstatus + editorial + fcriteria, function(err, result) {
            if(err == null && result != null && result.length > 0) {
    ...
    public static function findAllForOrder(orderId : DataIdentifier, fn : DataError -> Array<Post> -> Void) : Void {
    	Data.query('SELECT DISTINCT posts.* FROM posts INNER JOIN order_posts ON order_posts.post_id = posts.id WHERE order_posts.order_id = ?', [ orderId ], function(err, result : Array<Post>) {
            fn(err, result);
        });
    }
